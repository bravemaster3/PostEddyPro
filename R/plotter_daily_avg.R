...
#' @param window_size window size for centered rolling average, default is 15 days
    ...
    for (i in unique(df2$bloc)) {
      if (unique(df2[df2$bloc == i, "na_y"]) == 1) {
        g <- g + ggplot2::geom_line(data = df2[df2$bloc ==
                                                 i, ], ggplot2::aes(y = Var_smooth), alpha = 0.8, linewidth = 1.5)
      }
    }
  } else {
    g <- g + ggplot2::geom_line(data = df2, ggplot2::aes(y = Var_smooth), alpha = 0.8, linewidth = 1.5)
  }
