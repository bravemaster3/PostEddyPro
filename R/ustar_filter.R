...
ustar_filter <- function(fluxes_meteo,
  ...
  plot(ch4_flux_final~u_star, data=fluxes_meteo[which(fluxes_meteo$daytime==0),], xlim = c(0,0.8), ylim=c(-0.2, 0.2))
  plot(Temp~datetime, data=fluxes_meteo)

  #copying fluxes_meteo to keep it safe
  fluxes_meteo_copy <- fluxes_meteo[which(!is.na(fluxes_meteo$ch4_flux_final) & !is.na(fluxes_meteo$Temp)),]
  ...
  for (period in unique(fluxes_meteo_copy$period)){
    ...
    fluxes_meteo_copy_subset <- subset(fluxes_period, select = c("datetime", "ch4_flux_final", "u_star", "Temp"))
    ...
    names(u_df) <- c("Temp_class_ind", "ustar_class_ind", "Temp_class_max", "Temp_class_center", "ustar_class_max", "ustar_class_center", "ch4_avg", "prop_avg")
    ...
    for(i in 1:number_Temp_classes){
      try({
        ...
        plot(ch4_flux_final~u_star, data=Temp_subset[!is.na(Temp_subset$ch4_flux_final),], pch=16, cex=0.4, xlab="u*(m.s-1)",ylab = "CH4 flux (umol m-2 s-1)" )
        ...
        range_ustar <- range(Temp_subset$u_star[which(!is.na(Temp_subset$ch4_flux_final) & !is.na(Temp_subset$u_star))])
        ...
        for (j in 1:number_ustar_classes){
          ...
          if(j < number_ustar_classes) ch4_avg <- mean(Temp_subset$ch4_flux_final[which(Temp_subset$u_star>=ustar1 & Temp_subset$u_star<ustar2)], na.rm = T)

          if(j == number_ustar_classes) ch4_avg <- mean(Temp_subset$ch4_flux_final[which(Temp_subset$u_star>=ustar1 & Temp_subset$u_star<=ustar2)], na.rm = T)  #Without these conditions, we would will miss the last value

          #filling the table u_df
          inc=inc+1   #this increment will be used to fill the table correctly row-wise
          u_df[inc,] <- c(i, j, Temp2, Temp_mid, ustar2, ustar_mid, ch4_avg, NA)

          #plotting the points of average methane flux with the center value of the u_star class

          graphics::points(ustar2, ch4_avg, pch=16, cex=1, col="red")
          graphics::text(ustar2, ch4_avg, labels=round(ustar2,2), cex=0.5, font=2,col="green", srt=270, adj =-0.8)
          ...
          if(j>1){
            ...
            prop_avg <- u_df$ch4_avg[which(u_df$Temp_class_ind==i & u_df$ustar_class_ind==j-1)]/ch4_avg
            ...
            if(!is.na(prop_avg) & prop_avg >= 0.95) {#then we can plot those points in blue
              ...
              graphics::points(u_df$ustar_class_max[which(u_df$Temp_class_ind==i & u_df$ustar_class_ind==j-1)],
                     u_df$ch4_avg[which(u_df$Temp_class_ind==i & u_df$ustar_class_ind==j-1)],
