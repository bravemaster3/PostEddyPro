...
#' @export
visualization <- function (check_path_EC = TRUE, check_path_biomet = TRUE, path_EC = NULL,
                           path_biomet = NULL, EC_df = NULL, biomet_df = NULL, date_format = "%Y-%m-%d",
                           plot_co2 = TRUE, plot_ch4 = TRUE, ylim_co2 = c(-15, 15),
                           ylim_ch4 = c(-0.2, 0.2), ylim_h2o = c(-1.5, 8))
{
  if ((is.null(path_EC) & is.null(EC_df)) | (is.null(path_biomet) &
                                             is.null(biomet_df))) {
    stop("Both path_EC and EC_df, or both path_biomet and biomet_df cannot be missing at the same time")
  }
  if ((isTRUE(check_path_EC) & is.null(path_EC)) | (isTRUE(check_path_biomet) &
                                                    is.null(path_biomet))) {
  ...
  if (isTRUE(check_path_EC)) {
    # headers <- utils::read.csv(path_EC, skip = 1, header = FALSE,
    #                            nrows = 1, as.is = TRUE)

    # fluxes_site <- utils::read.table(path_EC, sep = ",",
    #                                  header = F, skip = 3)
    # colnames(fluxes_site) = headers
    # fluxes_site$date <- as.Date(fluxes_site$date, format = date_format)
    # fluxes_site$datetime <- as.POSIXct(paste(fluxes_site$date,
    #                                          fluxes_site$time, sep = " "), format = "%Y-%m-%d %H:%M",
    #                                    tz = "UTC")
    # fluxes_site[fluxes_site == -9999] <- NA
    # fluxes_site$datetime <- lubridate::round_date(fluxes_site$datetime,
    #                                               "30 minutes")

    ###
    ###
    headers <- data.table::fread(path_EC, skip = 1, header = FALSE,
                                 nrows = 1)
    headers <- as.character(headers[1, ])

    fluxes_site <- data.table::fread(path_EC, sep = ",",
                                     header = F, skip = 3)
  ...
  if (isTRUE(check_path_biomet)) {
    # meteo_site <- utils::read.table(path_biomet, sep = ",",
    #                                 header = T)
    meteo_site <- data.table::fread(path_biomet, sep = ",",
                                    header = T)
    meteo_site$datetime <- as.POSIXct(meteo_site$datetime,
  ...
  fluxes_meteo <- merge(fluxes_site, meteo_site, by = "datetime",
                        all = TRUE)
  ...
  h2o <- ggplot2::ggplot(data = fluxes_meteo, ggplot2::aes_string(x = "datetime",
                                                                  y = "h2o_flux")) + ggplot2::geom_point(size = 0.8) +
    ggplot2::geom_hline(yintercept = 0) + ggplot2::ylim(ylim_h2o) +
    ggplot2::theme_bw() + ggplot2::theme(axis.title.x = ggplot2::element_blank(),
                                         axis.text.x = ggplot2::element_blank())
  plot_heights = 1
  if (plot_co2 == TRUE) {
    co2 <- ggplot2::ggplot(data = fluxes_meteo, ggplot2::aes_string(x = "datetime",
                                                                    y = "co2_flux")) + ggplot2::geom_point(size = 0.8) +
      ggplot2::ylim(ylim_co2) + ggplot2::geom_hline(yintercept = 0) +
      ggplot2::theme_bw()
    if (plot_ch4 == TRUE) {
      co2 <- co2 + ggplot2::theme(axis.title.x = ggplot2::element_blank(),
                                  axis.text.x = ggplot2::element_blank())
      plot_heights = c(1, 1, 1.15)
    }
  }
  if (plot_ch4 == TRUE) {
    ch4 <- ggplot2::ggplot(data = fluxes_meteo, ggplot2::aes_string(x = "datetime",
                                                                    y = "ch4_flux")) + ggplot2::geom_point(size = 0.8) +
      ggplot2::xlab("date") + ggplot2::geom_hline(yintercept = 0) +
      ggplot2::ylim(ylim_ch4) + ggplot2::theme_bw()
  }
  if ((plot_ch4 + plot_co2) == 1)
    plot_heights = c(1, 1.15)
  if ((plot_ch4 + plot_co2) == 2) {
    plots <- cowplot::plot_grid(h2o, co2, ch4, align = "v",
                                ncol = 1, rel_heights = plot_heights)
  }
  else if (plot_ch4 == TRUE) {
    plots <- cowplot::plot_grid(h2o, ch4, align = "v", ncol = 1,
                                rel_heights = plot_heights)
  }
  else if (plot_co2 == TRUE) {
    plots <- cowplot::plot_grid(h2o, co2, align = "v", ncol = 1,
                                rel_heights = plot_heights)
  }
  else plots <- cowplot::plot_grid(h2o, align = "v", ncol = 1,
                                   rel_heights = plot_heights)
